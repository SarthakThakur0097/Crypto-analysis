...
def add_session_structure_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['time_since_session_open'] = df['session_step'] * 30  # assuming 30m interval
    ...
    df['prev_session_return'] = df.groupby('session')['close'].transform('last').shift(1) - df.groupby('session')['open'].transform('first').shift(1)
    df['prev_session_efficiency'] = df.groupby('session')['efficiency'].transform('mean').shift(1)
    return df
...
def add_candle_structure_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['engulfing_flag'] = (df['close'] > df['open'].shift(1)) & (df['open'] < df['close'].shift(1))
    df['strong_close'] = ((df['close'] - df['low']) / df['range'] > 0.8) | ((df['high'] - df['close']) / df['range'] > 0.8)
    return df
...
def add_trend_continuation_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['return_direction_match'] = np.sign(df['return']) == np.sign(df['prev_return'])
    df['rolling_return_sum_3'] = df['return'].rolling(window=3).sum()
...
def add_structure_zone_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['touch_prev_session_high'] = (df['high'] >= df['prev_high']) & (df['low'] <= df['prev_high'])
    df['touch_prev_session_low'] = (df['low'] <= df['prev_low']) & (df['high'] >= df['prev_low'])
    df['is_range_boundary_test'] = (df['close'] - df['rolling_max_high']) < (0.01 * df['close'])
    df['equal_highs_lows_flag'] = (df['high'].round(-1) == df['high'].shift(1).round(-1)) & (df['low'].round(-1) == df['low'].shift(1).round(-1))
    return df

def add_meta_composite_features(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    df['trend_strength'] = df['abs_return'] * df['efficiency']
    df['volatility_normalized_return'] = df['return'] / df['ATR_14']
    df['body_wick_alignment_score'] = (df['wick_top'] < df['body_ratio']) & (df['wick_bottom'] < df['body_ratio'])
    df['session_bias_score'] = df.groupby('session')['return'].transform(lambda x: x.rolling(3, min_periods=1).mean())
    return df
